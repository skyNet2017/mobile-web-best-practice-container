...
#ifndef INSTALLMENT_ANDROID_MD5_H
#define INSTALLMENT_ANDROID_MD5_H
...
* 最后逆序处理
*/
char *changeHex(char *str, unsigned int a[], const char *temp_str16) {
    int i, j;
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            int n = (a[i] >> (j * 2 * 4));
...
int md5_encode(const char *old_string, char *output, jboolean isLow) {
    ...
    unsigned int tmp[4] = {temp_a, temp_b, temp_c, temp_d};

    //char* md5_str = new char[32+1];
    changeHex(output, tmp, isLow ? STR16_LOW : STR16);
    return 0;
    //return md5_str;
}
